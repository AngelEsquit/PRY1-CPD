...
\begin{titlepage}
    ...
    \vspace{0.3cm}
    {\large Ing. Sebastian Galindo, Ing. Juan Luis Garcia\par}
    ...
    \vspace{0.3cm}
    {\large TonitoMC\par}
    {\large Javier-Espana\par}
    {\large AngelEsquit\par}

    \vfill

    {\large Guatemala, septiembre de 2026\par}
\end{titlepage}
...
para medir speedup. Usa Gauss-Seidel fila por fila para el solver y fuerza
bruta $O(f^2)$ para el n-body. No se modifica nunca despues de completada.
...
codigo de la rama \texttt{04-schedule-tuning}, reemplazando el
\texttt{schedule(dynamic, 16)} hardcodeado en todos los sitios del
catalogo. Al remedir la bateria estandar con el cambio real (no solo con
...
\texttt{dynamic, 16} anterior (B6: 88.27 $\to$ 80.68 FPS, $\sim -8.6\%$),
pero en malla grande gana fuerte (I6: 28.32 $\to$ 53.47 FPS,
$\sim +89\%$; C6: 45.79 $\to$ 68.57 FPS, $\sim +50\%$). Ver Tabla
\ref{tab:master} para los numeros completos.
...
\begin{longtable}{p{3.2cm}p{4.5cm}p{2.5cm}p{4cm}}
\toprule
...
\texttt{bh\_accumulate} & \texttt{idx}, \texttt{body} (int), \texttt{sources} (InkSource*), \texttt{acc\_x}, \texttt{acc\_y} (float*) & ninguna (acumula en \texttt{acc\_x}/\texttt{acc\_y}) & Recorre el arbol acumulando aceleracion gravitacional sobre una fuente. \\
