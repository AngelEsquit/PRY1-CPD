...
**This branch — `03-omp-solver`**: the pressure/diffusion solver (the
actual hotspot) is now parallel too, via a red-black Gauss-Seidel
checkerboard split. This is the single biggest jump in the whole
project — ~88.27 FPS at the default config (n=256, f=6) with 20 threads,
up from ~25.20 in `02-omp-loops` (~3.5x). Thread scaling also improves a
lot now that the real bottleneck is parallel (~6x at 20 threads, vs.
~2.6x before). See `docs/Roadmap.md` for the full benchmark table.
...
Everything from `02-omp-loops` (source injection, dissipation, advection,
projection, render), **plus** the solver itself:

`solve_linear` now uses **red-black Gauss-Seidel**: the grid is split
into a checkerboard by parity `(i+j) % 2`. All 4 neighbors of a cell are
always the opposite color, so every cell of one color can update in
parallel with zero internal dependencies — no more race condition. Each
iteration becomes: update all "red" cells (implicit barrier), update all
"black" cells (implicit barrier). The whole 20-iteration loop runs inside
a **single** `#pragma omp parallel` region (opening/closing a thread team
40 times a frame has real fixed cost; opening it once instead matters on
small grids), with `#pragma omp single` applying the boundary condition
between the two colors. Everything still uses
`schedule(dynamic, 16)`.

Red-black converges to essentially the same result as sequential
Gauss-Seidel (sometimes marginally faster), so the visual behavior
shouldn't look any different from `sequential`.
